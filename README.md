...
클릭한 쪽 목록의 스크롤은 그대로 유지됩니다(2.10.2~). 비활성 pane을 클릭하면 Obsidian이 그 pane을
활성화하며 스크롤을 건드리는데, 마우스 클릭에 한해 원래 위치로 되돌립니다. 키보드(Tab→Enter)로 화면 밖
줄을 열 때는 되돌리지 않습니다 — 그건 보이도록 스크롤되는 게 맞는 동작입니다.
...
| 버전 | 날짜 | 변경 사항 |
|------|------|----------|
| 2.10.2 | 2026-08 | 클릭으로 다른 노트를 열 때 클릭한 쪽 목록의 스크롤이 움직이던 문제 수정(마우스 클릭에 한해 원래 위치로 되돌림) |
